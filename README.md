...
Although it is not clear from the picture, "new user" button might trigger a pop-up new user form page.(In the picture, we are observing the phase after we clicked the button). 

then , the page is split to a user table (left) and new users form (right).This aims to adding new users and listing them to simutenously.
...
**Data Columns:**
    ...
    * Email
    * Enabled
Column headers are interactive.We should change the sort order by A single click (indicated by the arrow icons).
The funnel icons in the header row should trigger a column-specific filter.
...
!Mandatory fields are not clear on the picture.
...
5)Side panel closes.(Depends on usage frequency, the panel can either stay open for  several entries or close automatically to save space.)
6)User Table automatically refreshes and displays the new user.


Tunahan Gökgöz
tunagokgoz19@gmail.com
